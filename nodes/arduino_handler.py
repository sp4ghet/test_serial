...
if __name__ == '__main__':
    ...
    loop_rate_hz = rospy.get_param("~loop_rate_hz", 100)
    ...
    serial = Serial(serial_port_id, baud_rate)
    r = rospy.Rate(loop_rate_hz)
    # Read up to 128 bytes with 500ms timeout
    while not rospy.is_shutdown():
        timeout_s = 1 / loop_rate_hz
        buf = serial.read(serial_buffer_max, timeout_s)
        ...
        for environmental_variable, value in pairs:
            pub = Publisher(environmental_variable, Float64, queue_size=10)
            pub.publish(value)
        r.sleep()
    serial.close()